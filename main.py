...
import time
import sys
sys.setrecursionlimit(12825) # increase of recursion limit !
useridentify = sys.argv[2]
databaseid = sys.argv[3]
class EmailCrawler:
           ...
           #   print('Dumping processed urls to {}'.format(self.base_url.replace('.','_')+'.txt'))
                try : 
                   for url in self.processed_urls :
                        ...
                        sql = "INSERT INTO website_visited (url, date) VALUES (%s, %s)"
                        val = (url, DATETIME)
                        mycursor.execute(sql, val)   
                        mydb.commit()    
                except :
                     pass 
            #  print(mycursor.rowcount, " ------> urls inserted")
    def parse_url(self, current_url: str):   
        max_url =  20 
        #set the limit of url that can be crawled
        if len(self.processed_urls) < max_url : 
          """
                  ...
                  if email not in self.emails:
                      for url in self.processed_urls :
                          try :
                             ...
                             mycursor = mydb.cursor()
                             DATETIME = time.strftime('%Y-%m-%d %H:%M:%S')
                             sql = "INSERT INTO email_found (email, user_id, datase_id) VALUES (%s,%s,%s)"
                             val = (email, useridentify, databaseid)
                             mycursor.execute(sql, val)    
                             mydb.commit() 
                          except :
                                pass
